...
> 当前状态：备案已通过；准备上传 `0.1.3` 改名同步版；仍未正式发布。
...
```text
0.1.3：本地塘口经营记录体验版，同步最终小程序名称为“渔儿小助手”。支持塘口管理、投料/水质/用药/收获记录、本地 JSON 备份恢复、CSV 导出和关于与数据说明。当前已完成备案，仍不包含微信登录、支付、真实网络请求、上传、定位或 AppSecret。
```
