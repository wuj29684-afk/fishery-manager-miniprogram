...
> 当前状态：备案已通过；`0.2.1` 微信云开发账号同步文案一致性修正版已上传；准备验证/上传 `0.2.2` 首屏账号进入版；仍未正式发布。
...
```text
0.2.2：微信云开发账号同步体验版。新增首屏账号进入，支持进入时使用当前微信账号拉取账号数据，或把本机塘口和记录绑定到当前账号；保留塘口管理、投料/水质/用药/收获记录、本机 JSON 备份恢复和 CSV 导出。账号同步按微信云开发 OPENID 隔离；不包含支付、定位、文件上传、图片上传或 AppSecret。
```
