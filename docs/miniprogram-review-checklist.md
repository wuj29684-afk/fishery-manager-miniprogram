...
> 当前状态：备案已通过；`0.2.0` 微信云开发账号同步体验版已上传并通过用户真机体验；准备上传 `0.2.1` 文案一致性修正版；仍未正式发布。
...
- 小程序名称：渔儿小助手
- 当前定位：一线塘口经营值班台
- 当前技术边界：本机优先保存；用户主动账号同步时，通过微信云开发按当前微信账号隔离保存塘口和记录。不接支付、定位、文件上传、图片上传或 AppSecret。
...
```text
0.2.1：微信云开发账号同步体验版。支持塘口管理、投料/水质/用药/收获记录、本机 JSON 备份恢复、CSV 导出，以及在数据备份页主动绑定本机数据到当前微信账号或使用账号数据。账号同步按微信云开发 OPENID 隔离；不包含支付、定位、文件上传、图片上传或 AppSecret。
```
