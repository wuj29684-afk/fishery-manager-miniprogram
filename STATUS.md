...
- 版本状态：线上正式版为 `0.3.0`；`0.4.0` 已完成 Option 3 十二屏 UI、四类养殖单元图片、双仓验证、云函数部署和开发/体验版上传，并已提交微信代码审核
- 正式版：用户已确认 `0.3.0` 于 2026-07-23 审核通过并正式发布上线
- 当前候选：`0.4.0` 已恢复本机优先与可选登录边界，完成 Windows 微信开发者工具视觉验收；`syncAccountData` 已部署，开发/体验版已上传
- 正式审核/正式发布/微信认证：小程序备案与微信认证已通过；后续审核提交、正式发布、上传覆盖、扫码、验证码、人脸等敏感动作必须由用户当次确认或亲自操作
- 技术栈：Taro + React + TypeScript

## 当前下一步

1. 等待微信返回 `0.4.0` 代码审核结果。
2. 依赖审计保留上游风险记录，禁止强制降级或 `npm audit fix --force`。
3. 审核通过后的正式发布仍需用户当次单独明确确认。
