...
- 版本状态：线上正式版为 `0.2.6`；`0.2.8` 已提交审核并因首屏登录导向被驳回；`0.2.9` 首屏免登录修复版已在本地通过验证，尚未上传、尚未重新提交审核
...
1. 在微信开发者工具确认 `0.2.9` 打开即进入首页，体验示例、创建塘口和本机记录无需登录，账号同步仅位于“数据与设置”。
2. 经用户当次明确确认后覆盖上传 `0.2.9` 体验版并扫码回归；通过后再单独确认重新提交审核。
...
- 双仓 domain/about-data 检查、TypeScript、生产微信构建与云函数 `8/8` 测试通过；当前尚未上传 `0.2.9`、尚未重新提交审核、尚未正式发布。
