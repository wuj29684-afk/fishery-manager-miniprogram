...
- 版本状态：线上正式版仍为 `0.2.4`；`0.2.6` 已上传为开发/体验版本，并于 2026-07-11 19:32:05 提交微信代码审核，当前审核中
- 正式版：用户于 2026-07-08 告知 `0.2.4` 已正式发布上线
- 正式审核/正式发布/微信认证：小程序备案已通过；当前 `0.2.4` 已正式发布上线；用户于 2026-07-09 告知微信认证已通过；后续新版本提交审核、正式发布、扫码、验证码、人脸等敏感动作必须由用户确认或亲自操作
...
- 本次未部署云函数、未提交审核、未正式发布；正式线上版本仍为 `0.2.4`。
...
- 用户将自行后续提交审核；本次未提交审核、未正式发布，正式线上版本仍为 `0.2.4`。
...
- 当前状态为审核中，尚未正式发布；正式线上版本仍为 `0.2.4`。
- 后续仅在审核通过后，经用户当次明确确认，才可进行正式发布操作。

## 2026-07-11 0.2.7 体验示例同步保护（待上传）
...
- 该保护补丁将以 `0.2.7` 开发/体验版本上传，且不包含在已提交审核的 `0.2.6` 包中；审核状态保持“审核中”，不提交审核或正式发布。
