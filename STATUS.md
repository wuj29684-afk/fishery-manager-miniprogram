...
- 版本状态：线上正式版为 `0.2.6`；`0.2.8` 已于 2026-07-17 约 10:01 覆盖上传为开发/体验版本，尚未提交审核、尚未正式发布
...
1. 扫码完成 `0.2.8` 真机回归，覆盖登录、首页、塘口详情、塘口表单、记录表单、记录列表、数据设置和关于页，并确认账号同步与体验示例隔离未回归。
2. 回归通过后，再经用户当次明确确认提交 `0.2.8` 审核；审核通过后仍需单独确认正式发布。
3. 单独规划依赖安全加固，重点处理 `wx-server-sdk` 间接依赖的高风险安全公告；不得直接执行破坏性自动降级。
