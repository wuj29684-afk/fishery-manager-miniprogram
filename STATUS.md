...
- 版本状态：线上正式版为 `0.2.6`；`0.2.8` 已提交审核并因首屏登录导向被驳回；`0.2.9` 首屏免登录修复版已于 2026-07-20 约 10:44 覆盖上传体验版，尚未重新提交审核、尚未正式发布
...
1. 用户扫码回归 `0.2.9` 体验版：确认打开即进入首页，体验示例、创建塘口和本机记录无需登录，账号同步仅位于“数据与设置”。
2. 真机回归通过后，经用户当次单独确认再重新提交审核。
3. 审核通过后仍需单独确认正式发布；依赖安全加固继续作为独立任务处理。
...
- `0.2.9` 尚未重新提交审核、尚未正式发布；线上正式版仍为 `0.2.6`。
