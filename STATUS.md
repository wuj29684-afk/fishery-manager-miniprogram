...
- 版本状态：线上正式版为 `0.2.6`；`0.2.8` 已提交审核并因首屏登录导向被驳回；`0.2.9` 首屏免登录修复版已于 2026-07-20 约 10:44 覆盖上传体验版、20:10:59 重新提交审核，并于当日 20:33 前确认审核通过，当前待发布、尚未正式发布
...
1. 经用户当次单独确认后，才能在公众平台正式发布 `0.2.9`；在此之前线上版本仍为 `0.2.6`。
2. 正式发布后核对线上版本号与发布时间，再同步更新双仓状态。
3. 依赖安全加固继续作为独立任务处理。
...
- 经用户当次确认，`0.2.9` 已于 2026-07-20 20:10:59 重新提交审核；2026-07-20 20:33 前公众平台已显示“审核通过待发布”。该版本尚未正式发布，线上正式版仍为 `0.2.6`。
...
- 本次未提交微信代码审核、未正式发布；线上正式版本仍为 `0.2.6`。
