...
- 版本状态：线上正式版为 `0.2.9`；`0.3.0` 已于 2026-07-23 通过微信开发者工具 CLI 上传，用户于约 12:35 确认已提交微信代码审核，当前审核中
- 正式版：用户已确认 `0.2.9` 于 2026-07-20 正式发布上线
- 当前候选：`0.3.0` UI 与网箱业务重构已完成八屏视觉验收和双仓完整验证，已上传并提交审核，尚未正式发布；云函数改动尚未部署
- 正式审核/正式发布/微信认证：小程序备案与微信认证已通过；后续审核提交、正式发布、上传覆盖、扫码、验证码、人脸等敏感动作必须由用户当次确认或亲自操作
- 技术栈：Taro + React + TypeScript

## 当前下一步

1. 等待微信平台返回 `0.3.0` 代码审核结果。
2. 审核通过后，正式发布仍需用户当次单独确认；当前 `0.3.0` 云函数改动尚未部署，部署也需单独确认。
3. 依赖安全加固继续作为独立任务处理。
...
- 用户于 2026-07-23 约 12:35 确认已在微信公众平台提交 `0.3.0` 代码审核；当前等待审核结果，尚未正式发布，线上正式版仍为 `0.2.9`。
- 本轮未部署 `0.3.0` 云函数改动。
...
- 2026-07-20 约 20:57，用户确认已完成 `0.2.9` 正式发布；当前线上正式版为 `0.2.9`。
