...
- 版本状态：线上正式版为 `0.3.0`；本地 `0.4.0` 候选已完成 Option 3 十二屏 UI、四类养殖单元图片、双仓完整检查与生产构建，尚未上传体验版
- 正式版：用户已确认 `0.3.0` 于 2026-07-23 审核通过并正式发布上线
- 当前候选：`0.4.0` 已恢复本机优先与可选登录边界，完成 Windows 微信开发者工具视觉验收；云函数 `0.4.0` 源码仅在本地验证，尚未部署
- 正式审核/正式发布/微信认证：小程序备案与微信认证已通过；后续审核提交、正式发布、上传覆盖、扫码、验证码、人脸等敏感动作必须由用户当次确认或亲自操作
- 技术栈：Taro + React + TypeScript

## 当前下一步

1. 用户在微信开发者工具或真机体验当前 `0.4.0` 候选，重点确认空白首页、五栏导航、四类养殖单元图片、快记表单和数据同步页。
2. 依赖安全加固继续作为独立任务处理，禁止直接自动强制升级。
3. 上传体验版、部署云函数、提交审核和正式发布均需用户当次分别确认。
