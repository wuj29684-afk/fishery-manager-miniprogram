...
- 版本状态：`0.4.0` 为当前线上正式版；`0.4.1` 体验版已上传，当前进入后续审核准备阶段
- 正式版：当前线上正式版为 `0.4.0`
- 当前候选：`0.4.1` 体验版已上传，修复新增养殖单元误报已有进行中批次问题
- 正式审核/正式发布/微信认证：小程序备案与微信认证已通过；后续审核提交、正式发布、上传覆盖、扫码、验证码、人脸等敏感动作必须由用户当次确认或亲自操作
- 技术栈：Taro + React + TypeScript

## 当前下一步

1. 用户体验 `0.4.1` 体验版并决定是否提交审核。
2. 提交审核和正式发布仍需用户当次明确确认。
3. 依赖审计保留上游风险记录，禁止强制降级或 `npm audit fix --force`。

## 2026-07-26 0.4.1 体验版上传

- 上传时间：2026-07-26 11:16（Asia/Shanghai）。
- 上传版本：`0.4.1`。
- 上传备注：`pond-create-fix: select new unit before starting batch`。
- 修复新建养殖单元后仍使用旧单元创建批次，导致误报“已有进行中批次”的问题。
- 双仓检查、TypeScript、生产构建和微信开发者工具真实创建回归均已通过。
- 当前仅上传体验版，尚未提交审核或正式发布。

## 2026-07-26 0.4.0 正式发布与线上 Bug 调试

- 用户确认 `0.4.0` 已审核通过并正式发布，当前线上正式版更新为 `0.4.0`。
- 启动阶段不再自动初始化或同步 CloudBase，账号能力改为用户主动登录/同步时懒初始化。
- 图标字体改为本地资源；修复首页误导状态、记录日期、筛选缺项、创建步骤、水质字段映射与空值写零问题。
- “保存草稿”现可真实保存并恢复；开发者工具自动化验证草稿与水质保存通过，测试数据已恢复。
- 首页、建档、现场记录与报告统一使用设备本地日期，修复凌晨记录可能落到前一天的问题。
- 本仓通过 domain、about-data、v4、v4-ui、TypeScript、生产构建和 CloudBase 15 项测试。
- 本轮尚未上传体验版、未提交审核、未正式发布新的修复版本。

## 2026-07-24 0.4.0 云函数部署、体验版上传与提交审核

- `syncAccountData` 已部署到 `cloud1-d0gae5atcb0f634b3`，共 4 个文件，云端安装依赖，部署包约 `19.3 KB`。
- 微信开发者工具 CLI 已成功上传 `0.4.0`，AppID 为 `wxa516d96010f19988`，包体 `1,461,170` 字节，上传备注 `v040-option3: twelve-screen UI and unified unit imagery`。
- 上传前主仓重新通过 domain、about/data、v4、v4-ui、TypeScript、生产构建和 CloudBase 15 项测试；双仓源码与此前验证候选一致。
- 用户于 2026-07-24 约 21:19 确认已在微信公众平台提交 `0.4.0` 代码审核。
- 当前等待微信审核结果，尚未正式发布，线上正式版仍为 `0.3.0`。

## 2026-07-24 0.4.0 Option 3 十二屏与养殖单元图片验收通过

- 主仓 `miniprogram/` 的 `0.4.0` 候选已完整镜像到本仓，包含 v3 运行时、十二屏 UI、可选登录/本机优先流程和四类养殖单元统一图片。
- 网箱、塘口、室内池、其他图片在创建页、列表缩略图和详情大图中完成微信开发者工具裁切验收；室内池明确展示循环水圆池、过滤设备与管路。
- `qa-captures/design-vs-implementation-option3.jpg` 为最终十二屏对照，`design-qa.md` 结论为 `final result: passed`。
- 本仓通过 domain、about/data、v4、v4-ui、TypeScript、生产构建和 CloudBase 15 项测试；与主仓 167 个交付文件 SHA-256 一致，两个构建产物均为 `1,479,753` 字节。
- 本轮未上传微信体验版、未部署云函数、未提交审核、未正式发布；线上正式版仍为 `0.3.0`。
...
## 2026-07-23 0.3.0 八屏 UI 与网箱业务重构、上传与提审

- 严格采用已确认设计图，完成五栏导航、双模式首页、养殖单元列表、网箱建档、五页签详情、投喂/水质中量表单和记录/设置合并页。
- 网箱数据模型新增尺寸、投放数量和网箱鱼预警模板；首页、详情、记录和同步协议继续保留塘口与网箱兼容。
- 微信开发者工具 iPhone 12/13 八屏截图已完成同画面对照，`design-qa.md` 结论为 `final result: passed`。
- 小程序包版本、锁文件和“关于渔儿小助手”页面统一升级为 `0.3.0`；关于页改为直接读取包版本，检查脚本会阻止版本号再次失配。
- “关于渔儿小助手”已在全新微信开发者工具项目中真实编译验收，页面显示 `当前版本 v0.3.0`；证据截图为 `qa-captures/09-about-version-030.png`。
- 双仓 domain、about/data、TypeScript、生产构建和云函数 9 项测试全部通过；66 个源码、脚本、云函数与设计文件哈希一致。
- 经用户当次确认，已通过微信开发者工具 CLI 上传 `0.3.0`，AppID 为 `wxa516d96010f19988`，包体约 `938.6 KB`，上传备注 `v030-ui-cage: five-tab UI and pond cage workflows`。
- 用户于 2026-07-23 约 12:35 确认已在微信公众平台提交 `0.3.0` 代码审核；随后确认审核通过并完成正式发布，`0.3.0` 成为线上正式版。
- 本轮未部署 `0.3.0` 云函数改动。

## 2026-07-20 0.2.8 审核驳回与 0.2.9 首屏免登录修复

- 用户提供的微信审核结果显示：`0.2.8` 因打开后先展示登录导向、未先浏览体验功能而未通过审核。
- `0.2.9` 已将 `pages/index/index` 设为首屏并移除独立登录页；空白首页可直接打开本地体验示例或创建塘口。
- 账号同步仅保留在“数据与设置”页，并明确本机浏览和记录无需登录，只有用户主动选择时才使用当前微信账号同步。
- 双仓 domain/about-data 检查、TypeScript、生产微信构建与云函数 `8/8` 测试通过。
- 经用户当次确认，2026-07-20 约 10:44 已通过微信开发者工具覆盖上传 `0.2.9` 体验版，上传备注 `review-entry-fix: browse before optional account sync`；开发者工具显示“代码上传成功”，首屏路径为 `pages/index/index`。
- 经用户当次确认，`0.2.9` 已于 2026-07-20 20:10:59 重新提交审核；2026-07-20 20:33 前公众平台已显示“审核通过待发布”。
- 2026-07-20 约 20:57，用户确认已完成 `0.2.9` 正式发布；该版本在当时成为线上正式版。

## 2026-07-17 0.2.8 候选 6 全量界面重设计体验版

- 候选 6 已覆盖登录、首页、塘口详情、塘口表单、记录表单、记录列表、数据设置和关于页，并新增统一 `AppTabBar`、品牌图与塘口场景图。
- 双仓包内版本、关于页和版本检查已同步为 `0.2.8`；双仓 `check:about-data`、TypeScript 与生产微信构建通过。
- 经用户当次确认，2026-07-17 约 10:01 已通过微信开发者工具覆盖上传 `0.2.8` 开发/体验版本，备注 `candidate6-redesign: full UI refresh and version sync`。
- 该次 `0.2.8` 上传未提交代码审核、未正式发布；操作发生时线上正式版本为 `0.2.6`。
...
- 此条为 `0.2.5` 阶段历史记录：当时尚未真机验证或提交审核；在后续 `0.2.6` 提交审核时，线上正式版为 `0.2.4`。
...
- 该保护补丁不包含在已提交审核并于 2026-07-12 正式上线的 `0.2.6` 包中；截至该条记录，`0.2.7` 尚未提交审核或正式发布。
...
- 用户于 2026-07-12 完成 `0.2.6` 正式发布；该版本在当时成为线上正式版本。
- 截至该条记录，主仓的 `0.2.7` 是未提交审核的开发/体验版本，未改变 `0.2.6` 当时的正式线上状态。
